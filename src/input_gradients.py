...
def optimize_proba_wrt_data(
    ...
    - trajectory[idx] is a list containing config["save_k_intermediate_imgs"] images as tensors. Each
    tensor has shape (batch_size, *data.shape).
    - objectives[idx] is a list containing config["num_steps"] tensors of shape (batch_size,). Each
    value is the objective (by default, the probability of the target class) at the corresponding step.
    - grad_norms[idx] is a list containing config["num_steps"] tensors of shape (batch_size,). Each
    value is the gradient l2 norm at the corresponding step.
    ...
    a list containing config["save_k_intermediate_imgs"] tensors. Each tensor has shape
    (batch_size, *latent.shape) and contains the latent embeddings at the corresponding step.
    ...
    for target in targets:
        ...
        for step in range(config["num_steps"]):
            ...
            if step == 0 or (step + 1) % config["save_every_k"] == 0:
                trajectories[target].append(x.cpu().detach().clone())
...
def _optimize_proba_wrt_data_with_autoencoder(
    ...
    for target in targets:
        ...
        for step in range(config["num_steps"]):
            ...
            if step == 0 or (step + 1) % config["save_every_k"] == 0:
                trajectories[target].append(x.cpu().detach().clone())
                latent_trajectories[target].append(z.cpu().detach().clone())
            objectives[target].append(probas.detach().cpu().clone())
            grad_norms[target].append(
                (z.grad**2).mean(dim=(1, 2, 3)).sqrt().cpu().detach().clone()
            )
...
def plot_optimization_trajectory_fixed_target(
    out: dict, target: int, class_names: Optional[list | dict] = None, figsize=(15, 8)
):
    """
    ...
    num_steps = len(probas)
    batch_size = len(probas[target][0])
    fig1, axes1 = plt.subplots(1, 2, figsize=figsize, squeeze=True)
    ...
    fig2, axes2 = plt.subplots(
        2 * batch_size, num_frames, figsize=figsize, squeeze=False
    )
    for j, img in enumerate(trajectory[target]):
        for i in range(batch_size):
            axes2[2 * i, j].imshow(prepare_tensor_image_for_plot(img[i]))
            title = (
                f"M={img[i].abs().max().item():.2f}p={probas[target][j, i].item():.2f}"
            )
            axes2[2 * i, j].set_title(title)
            axes2[2 * i, j].axis("off")
            diff = trajectory[target][j][i] - trajectory[target][0][i]
...
def plot_optimization_trajectory_fixed_sample(
    ...
    fig2, axes2 = plt.subplots(
        2 * len(targets), num_frames, figsize=figsize, squeeze=False
    )
    for i, target in enumerate(targets):
        for j, img in enumerate(trajectory[target]):
            axes2[2 * i, j].imshow(prepare_tensor_image_for_plot(img[sample_idx]))
            title = f"M={img[sample_idx].abs().max().item():.2f}p={probas[target][j, sample_idx].item():.2f}"
            axes2[2 * i, j].set_title(title)
            axes2[2 * i, j].axis("off")
            diff = trajectory[target][j][sample_idx] - trajectory[target][0][sample_idx]
            axes2[2 * i + 1, j].imshow(prepare_tensor_image_for_plot(diff))
            axes2[2 * i + 1, j].set_title(f"M={diff.abs().max().item():.2f}")
            axes2[2 * i + 1, j].axis("off")
...
def plot_optimal_images(
    ...
    data = out["trajectory"][targets[0]][0]
    if class_names is None:
        class_names = {idx: str(idx) for idx in out["final_imgs"].keys()}
    batch_size = len(data)
    fig, axes = plt.subplots(
        2 * batch_size, len(targets), figsize=figsize, squeeze=False
    )
    for i in range(batch_size):
        ...
        for j, target in enumerate(targets):
            ...
            title = f"{class_names[target]}\nM={opt[target][i].abs().max().item():.2f}"
